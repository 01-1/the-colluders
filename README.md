...
Model calls use OpenRouter only when `OPENROUTER_API_KEY` is present in a local `.env` file:

```text
OPENROUTER_API_KEY=...
```

If no key is present, model rooms stay visible but show an offline state. Human multiplayer remains fully playable.
